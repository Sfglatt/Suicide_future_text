...
transcripts <- transcripts %>%
  mutate(participant_id = tools::file_path_sans_ext(basename(doc_id)),
         group = case_when(
           as.numeric(gsub("[^0-9]", "", participant_id)) < 95 ~ "suicidal",
           as.numeric(gsub("[^0-9]", "", participant_id)) >= 95 ~ "not_suicidal",
           TRUE ~ NA_character_
         ))
...
(bing_sentiments <- tidy_transcripts %>%
  ...
  mutate(sentiment = positive - negative))
# Don't forget that this will change based on how *many* people are in each group! e.g., I currently have 2 non-suicidal and 7 suicidal, so since sentiment is based on counts, it will be skewed. 
