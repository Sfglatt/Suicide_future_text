...
```{r Data}
# Use 'transcripts' from the text processing / sentiment file. This has participants' transcripts and suicide risk status. 
...
tidy_text_stem_stop %>% group_by(stem) %>% 
  summarise(count = n()) %>% 
  arrange(desc(count)) %>% 
  head(10)
```
...
lasso_rs_predictions %>%
  group_by(id) %>%
  roc_curve(truth = group, .pred_1, event_level = "second") %>%
  autoplot() +
  labs(
    color = NULL,
...
lasso_rs_predictions %>%
  group_by(id) %>%
  pr_curve(truth = group, .pred_1, event_level = "second") %>%
  autoplot()+
  labs(
    color = NULL,
...
final_imp %>%
  ...
  labs(
    ...
    subtitle = paste0("These features are the most important in predicting\n",
                      "whether descriptions of the future self are from people at risk or not")
  )
```
